"""
modules.supply_chain.ledger — 交易账本 + 数字签名
─────────────────────────────────────────────────
职责：
  1. 撮合成交后自动生成交易流水（TransactionLedger 记录）
  2. 路由费计算：成交金额 * fee_rate（默认 1%）
  3. SHA256 数字签名：对流水核心字段做防篡改哈希
  4. 提供账本查询接口
"""
...
class LedgerService:
    ...
    - signature: SHA256(transaction_id|merchant_id|amount|fee|timestamp)
    """
    ...
    def create_transaction(
        ...
        sign_payload = f"{txn_id}|{merchant_id}|{amount_usd}|{fee}|{ts}"
        ...
        logger.info(
            "账本流水: txn=%s merchant=%s amount=$%.2f fee=$%.2f sig=%s…",
            txn_id[:8], merchant_id[:8], amount_usd, fee, signature[:12],
        )
        return record
    ...
    def verify_signature(self, record: dict[str, Any]) -> bool:
        """验证流水签名是否被篡改"""
        sign_payload = (
            f"{record['transaction_id']}|{record['merchant_id']}|"
